...
export function ChatInput({
  ...
  const sendShortcut = isMacOS ? 'Ctrl+Enter' : 'Alt+Enter'
  ...
  const handleMessageChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (textareaRef.current) {
      ...
      const borderRadius = newHeight <= 46 ? '12px' : `${Math.max(8, 12 - (newHeight - 46) / 10)}px`
        ...
        <Tooltip content={
          ...
          <div className="flex-shrink-0 w-8 h-8">
            <button
              ...
              className="w-full h-full rounded-full flex items-center justify-center text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-lg"
              style={{
